...
namespace VRTK
{
    ...
    public class VRTK_HeightAdjustTeleport : VRTK_BasicTeleport
    {
		...
		/// TODO: Adjust y to offset to ship
		/// </summary>
		/// <param name="tipPosition">The tip position.</param>
		...
		protected override Vector3 GetNewPosition(Vector3 tipPosition, Transform target)
        {
			...
			//base position should offset
			basePosition = GetTeleportOffset(target, tipPosition);
			//basePosition = GetTeleportY(target, tipPosition);
			return basePosition;
        }
		...
		/// <summary>
		/// Gets the teleport y.
		/// Major change here since y needs to be rotated.
		/// </summary>
		/// <param name="target">The target.</param>
		/// <param name="tipPosition">The tip position.</param>
		/// <returns></returns>
        ...
        {
			//um, no, we need to offset by rotation!
			//Debug.Log(target.name);
			var newY = (transform.position).y;
			//var newY = (transform.rotation * transform.position).y;
		...
		/// <param name="tipPosition">The tip position.</param>
		/// <returns></returns>
		private Vector3 GetTeleportOffset(Transform target, Vector3 tipPosition)
		{
			// um, no, we need to offset by rotation!
			// Debug.Log(target.name);
			//if shit doesnt work out reset origin.
			var newposition = tipPosition; //(Quaternion.Inverse(transform.rotation) * transform.position);
			var heightOffset = 0.1f;
			//Check to see if the tip is on top of an object
			var rayStartPositionOffset = transform.up * heightOffset;
			
			var ray = new Ray(tipPosition + rayStartPositionOffset, -transform.up);
			RaycastHit rayCollidedWith;
			if (target && Physics.Raycast(ray,out rayCollidedWith, 1, mask))
			{

				//newposition = 
				//	(tipPosition - transform.up * rayCollidedWith.distance) 
				//		+ transform.up * heightOffset;

				//var g0 = Instantiate(debugger, (tipPosition), Quaternion.identity);
				//g0.name = "tipPosition";

				//var g1 = Instantiate(debugger, (tipPosition - transform.up * rayCollidedWith.distance), Quaternion.identity);
				//g1.name = "tipPosition - transform.up * rayCollidedWith.distance";

				//var g2 = Instantiate(debugger, newposition, Quaternion.identity);
				//g2.name = "newposition";
            }
